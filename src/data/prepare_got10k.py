...
AUC). This script:
  ...
  2. shuffles and greedily adds whole sequences until the cumulative image-byte
     ...
     stays meaningful,
  3. splits the selected sequences into train / val (for training the
     preprocessor vs. reporting AUC), and writes an index JSON.
...
-----
    python -m src.data.prepare_got10k \
        ...
        --out data/index/got10k_3gb.json --cap-gb 3 --val-frac 0.3
"""
...
def build_index(
    ...
    n_val = max(1, int(len(selected) * val_frac)) if len(selected) > 1 else 1
    val = selected[:n_val]
    train = selected[n_val:] or selected  # never leave train empty
...
def main() -> None:
    ...
    a = p.parse_args()
    build_index(a.root, a.out, a.cap_gb, a.val_frac, a.seed)
