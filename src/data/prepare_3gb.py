...
out as ``<root>/<split>/<class_name>/<video>.mp4``. This script:
  ...
  4. splits the selection train/val and writes an index JSON.
...
def build_index(
    ...
    n_val = int(len(selected) * val_frac)
    val = selected[:n_val]
    ...
    print(
        ...
        f"across {m['n_classes']} classes -> train {m['n_train']} / val {m['n_val']}"
    )
    if unmapped:
...
def main() -> None:
    ...
    a = p.parse_args()
    build_index(a.root, a.out, a.cap_gb, a.val_frac, a.backbone, a.seed)
